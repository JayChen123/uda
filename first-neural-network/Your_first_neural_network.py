...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            # final_outputs = self.activation_function(final_inputs)  output layer no need akt
...
# 
# #### 请将你的答案填写在下方
# 预测结果良好，模型几乎没有过拟合，在训练集的loss与验证集的loss非常接近。下面总结一下我的感受
# 1. 在实现神经网络的反向传播，更新权重的时候，我理解出了问题，我一直认为输出层也需要激活函数，
# 结果总是过不了单元测试，开始找遍了网上的资料试图自己解决，还是没能发现这个问题。之后找了别人的作业，
# 看了代码，比对了好久才发现这个问题，原来是我忽略了这个细节。
# 2. 调试超参的一点经验。其实这些参数都是有关联的，调参采用控制变量法，每次只调整单个参数。并且要记录每组
# 参数对应的结果。比如总迭代次数，先取较小的学习率，迭代次数一般从100开始，如果发现loss曲线还在往下降，
# 再逐渐增加迭代次数，如果发现loss曲线开始震荡稳定，此时拟合效果不好时，说明学习率不合适了，一般情况下
# 会较少或者增加一个数量级的学习率，继续尝试。至于隐层节点数，主要看拟合效果，如果过拟合那就减少隐层个数，反之就
# 增加隐层个数。当然调参是非常复杂的，这里也只能简单说一点。
# 3. 个人认为模型参数不是最好的，其实还有优化空间，模型在预测出错集中在月末的10天，可以进一步排查数据或者模型问题，
# 提升预测精度。
